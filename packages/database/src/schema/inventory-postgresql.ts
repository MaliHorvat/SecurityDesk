...
import { customer, site, manufacturer, device } from "./securitydesk-postgresql";
...
export const inventoryCategory = pgTable(
  "inventory_category",
  {
    ...
    parentId: varchar("parent_id", { length: 36 }).references(
      (): AnyPgColumn => inventoryCategory.id,
      { onDelete: "set null" },
    ),
...
export const inventoryLocation = pgTable(
  "inventory_location",
  {
    ...
    parentId: varchar("parent_id", { length: 36 }).references(
      (): AnyPgColumn => inventoryLocation.id,
      { onDelete: "set null" },
    ),
...
export const purchaseOrderItem = pgTable(
  "purchase_order_item",
  {
    ...
    quantityOrdered: doublePrecision("quantity_ordered").notNull().default(0),
...
export const stocktakeItem = pgTable(
  "stocktake_item",
  {
    ...
    expectedQuantity: doublePrecision("expected_quantity").notNull().default(0),
