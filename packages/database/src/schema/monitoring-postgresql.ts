...
import { site, device } from "./securitydesk-postgresql";

export const monitoringAgentStatusEnum = pgEnum("monitoring_agent_status", [
  "pending",
  "active",
  "offline",
  "revoked",
...
export const monitoringCheckStatusEnum = pgEnum("monitoring_check_status", [
  "online",
  "offline",
  "degraded",
  "error",
...
export const monitoringAgent = pgTable(
  "monitoring_agent",
  {
    ...
    status: monitoringAgentStatusEnum("status").notNull().default("pending"),
...
export const agentEnrollmentToken = pgTable(
...
export const monitoringCheck = pgTable(
  "monitoring_check",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    organizationId: varchar("organization_id", { length: 36 })
    ...
    name: varchar("name", { length: 255 }).notNull(),
    checkType: monitoringCheckTypeEnum("check_type").notNull().default("ping"),
...
export const monitoringCheckResult = pgTable(
  "monitoring_check_result",
  {
    ...
    status: monitoringCheckStatusEnum("status").notNull().default("unknown"),
