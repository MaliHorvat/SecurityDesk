...
export const networkPortStatusEnum = pgEnum("network_port_status", [
  "up",
  "down",
  "disabled",
  "error",
...
export const networkPortRoleEnum = pgEnum("network_port_role", [
  "access",
  "trunk",
  "uplink",
  "unused",
...
export const networkPoeStateEnum = pgEnum("network_poe_state", [
  "off",
  "on",
  "fault",
  "denied",
...
export const networkSwitch = pgTable(
  "network_switch",
  {
    ...
    poeBudgetWatts: doublePrecision("poe_budget_watts").notNull().default(0),
...
export const networkVlan = pgTable(
  "network_vlan",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    organizationId: varchar("organization_id", { length: 36 })
...
export const networkPort = pgTable(
  "network_port",
  {
    id: varchar("id", { length: 36 }).primaryKey(),
    organizationId: varchar("organization_id", { length: 36 })
    ...
    description: text("description"),
    status: networkPortStatusEnum("status").notNull().default("unknown"),
    role: networkPortRoleEnum("role").notNull().default("unused"),
    speedMbps: integer("speed_mbps"),
    duplex: varchar("duplex", { length: 16 }),
    poeState: networkPoeStateEnum("poe_state").notNull().default("unknown"),
    poeWatts: doublePrecision("poe_watts").notNull().default(0),
...
export const networkIpAssignment = pgTable(
